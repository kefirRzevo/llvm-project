...
#define DEBUG_TYPE "riscs-inst-info"
...
static riscsCC::CondCode getCondFromBranchOpc(unsigned Opc) {
  switch (Opc) {
  default:
    return riscsCC::COND_INVALID;
  case riscs::BEQ:
    return riscsCC::COND_EQ;
  case riscs::BNE:
    return riscsCC::COND_NE;
  case riscs::BLT:
    return riscsCC::COND_LT;
  case riscs::BLTU:
    return riscsCC::COND_LTU;
  case riscs::BGE:
    return riscsCC::COND_GE;
  case riscs::BGEU:
    return riscsCC::COND_GEU;
  }
}
...
const MCInstrDesc &RISCSInstrInfo::getBrCond(riscsCC::CondCode CC) const {
  switch (CC) {
  default:
    llvm_unreachable("Unknown condition code!");
  case riscsCC::COND_EQ:
    return get(riscs::BEQ);
  case riscsCC::COND_NE:
    return get(riscs::BNE);
  case riscsCC::COND_LT:
    return get(riscs::BLT);
  case riscsCC::COND_GE:
    return get(riscs::BGE);
  case riscsCC::COND_LTU:
    return get(riscs::BLTU);
  case riscsCC::COND_GEU:
    return get(riscs::BGEU);
  }
}

riscsCC::CondCode riscsCC::getOppositeBranchCondition(riscsCC::CondCode CC) {
  switch (CC) {
  default:
...
MachineBasicBlock *
RISCSInstrInfo::getBranchDestBlock(const MachineInstr &MI) const {
...
void RISCSInstrInfo::loadRegFromStackSlot(
  ...
  BuildMI(MBB, I, DL, get(riscs::LD), DstReg)
      .addFrameIndex(FI)
      .addImm(0)
      .addMemOperand(MMO);
}

bool RISCSInstrInfo::reverseBranchCondition(
    SmallVectorImpl<MachineOperand> &Cond) const {
  assert((Cond.size() == 3) && "Invalid branch condition!");
  auto CC = static_cast<riscsCC::CondCode>(Cond[0].getImm());
  Cond[0].setImm(getOppositeBranchCondition(CC));
  return false;
}

unsigned RISCSInstrInfo::insertBranch(
  ...
  if (Cond.empty()) {
    MachineInstr &MI = *BuildMI(&MBB, DL, get(riscs::PseudoBR)).addMBB(TBB);
    if (BytesAdded)
  ...
  MachineInstr &MI = *BuildMI(&MBB, DL, get(riscs::PseudoBR)).addMBB(FBB);
  if (BytesAdded)
    *BytesAdded += getInstSizeInBytes(MI);
  return 2;
}

unsigned RISCSInstrInfo::getInstSizeInBytes(const MachineInstr &MI) const {
  llvm_unreachable("");
}
