...
std::optional<MCFixupKind> RISCSAsmBackend::getFixupKind(StringRef Name) const {
  if (STI.getTargetTriple().isOSBinFormatELF()) {
    unsigned Type;
    Type = llvm::StringSwitch<unsigned>(Name)
#define ELF_RELOC(X, Y) .Case(#X, Y)
#include "llvm/BinaryFormat/ELFRelocs/RISCS.def"
#undef ELF_RELOC
               .Case("BFD_RELOC_NONE", ELF::R_RISCS_NONE)
               .Case("BFD_RELOC_32", ELF::R_RISCS_32)
               .Case("BFD_RELOC_64", ELF::R_RISCS_64)
               .Default(-1u);
    if (Type != -1u)
...
RISCSAsmBackend::getFixupKindInfo(MCFixupKind Kind) const {
  const static MCFixupKindInfo Infos[] = {
      ...
      {"fixup_RISCS_hi20", 12, 20, 0},
      {"fixup_RISCS_lo12_i", 20, 12, 0},
      {"fixup_RISCS_lo12_s", 0, 32, 0},
      {"fixup_RISCS_pcrel_hi20", 12, 20,
       MCFixupKindInfo::FKF_IsPCRel | MCFixupKindInfo::FKF_IsTarget},
      {"fixup_RISCS_pcrel_lo12_i", 20, 12,
       MCFixupKindInfo::FKF_IsPCRel | MCFixupKindInfo::FKF_IsTarget},
      {"fixup_RISCS_pcrel_lo12_s", 0, 32,
       MCFixupKindInfo::FKF_IsPCRel | MCFixupKindInfo::FKF_IsTarget},
      {"fixup_RISCS_got_hi20", 12, 20, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_RISCS_tprel_hi20", 12, 20, 0},
      {"fixup_RISCS_tprel_lo12_i", 20, 12, 0},
      {"fixup_RISCS_tprel_lo12_s", 0, 32, 0},
      {"fixup_RISCS_tprel_add", 0, 0, 0},
      {"fixup_RISCS_tls_got_hi20", 12, 20, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_RISCS_tls_gd_hi20", 12, 20, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_RISCS_jal", 12, 20, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_RISCS_branch", 0, 32, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_RISCS_call", 0, 64, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_RISCS_call_plt", 0, 64, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_RISCS_relax", 0, 0, 0},
      {"fixup_RISCS_align", 0, 0, 0},

      {"fixup_RISCS_set_8", 0, 8, 0},
      {"fixup_RISCS_add_8", 0, 8, 0},
      {"fixup_RISCS_sub_8", 0, 8, 0},

      {"fixup_RISCS_set_16", 0, 16, 0},
      {"fixup_RISCS_add_16", 0, 16, 0},
      {"fixup_RISCS_sub_16", 0, 16, 0},

      {"fixup_RISCS_set_32", 0, 32, 0},
      {"fixup_RISCS_add_32", 0, 32, 0},
      {"fixup_RISCS_sub_32", 0, 32, 0},

      {"fixup_RISCS_add_64", 0, 64, 0},
      {"fixup_RISCS_sub_64", 0, 64, 0},

      {"fixup_RISCS_set_6b", 2, 6, 0},
      {"fixup_RISCS_sub_6b", 2, 6, 0},
  };
  static_assert((array_lengthof(Infos)) == riscs::NumTargetFixupKinds,
                "Not all fixup kinds added to Infos array");

  // Fixup kinds from .reloc directive are like R_RISCS_NONE. They
...
bool RISCSAsmBackend::shouldForceRelocation(const MCAssembler &Asm,
                                            const MCFixup &Fixup,
...
bool RISCSAsmBackend::fixupNeedsRelaxationAdvanced(
...
bool RISCSAsmBackend::relaxDwarfLineAddr(const MCAssembler &Asm,
  ...
  if (Value > 60000) {
    ...
    Offset = OS.tell();
    Fixup = {riscs::fixup_RISCS_add_16, riscs::fixup_RISCS_sub_16};
...
bool RISCSAsmBackend::relaxDwarfCFA(const MCAssembler &Asm,
  ...
  if (isUIntN(6, Value)) {
    ...
    AddFixups(0, {riscs::fixup_RISCS_set_6b, riscs::fixup_RISCS_sub_6b});
  } else if (isUInt<8>(Value)) {
    OS << uint8_t(dwarf::DW_CFA_advance_loc1);
    support::endian::write<uint8_t>(OS, 0, endianness::little);
    AddFixups(1, {riscs::fixup_RISCS_set_8, riscs::fixup_RISCS_sub_8});
  } else if (isUInt<16>(Value)) {
    OS << uint8_t(dwarf::DW_CFA_advance_loc2);
    support::endian::write<uint16_t>(OS, 0, endianness::little);
    AddFixups(1, {riscs::fixup_RISCS_set_16, riscs::fixup_RISCS_sub_16});
  } else if (isUInt<32>(Value)) {
    OS << uint8_t(dwarf::DW_CFA_advance_loc4);
    support::endian::write<uint32_t>(OS, 0, endianness::little);
    AddFixups(1, {riscs::fixup_RISCS_set_32, riscs::fixup_RISCS_sub_32});
  } else {
...
bool RISCSAsmBackend::evaluateTargetFixup(const MCAssembler &Asm,
                                          const MCFixup &Fixup,
                                          const MCFragment *DF,
                                          const MCValue &Target,
                                          const MCSubtargetInfo *STI,
  ...
  case riscs::fixup_RISCS_pcrel_lo12_i:
  case riscs::fixup_RISCS_pcrel_lo12_s: {
    AUIPCFixup = cast<RISCSMCExpr>(Fixup.getValue())->getPCRelHiFixup(&AUIPCDF);
    if (!AUIPCFixup) {
...
bool RISCSAsmBackend::shouldInsertExtraNopBytesForCodeAlign(
    const MCAlignFragment &AF, unsigned &Size) {
  // Calculate Nops Size only when linker relaxation enabled.
  return false;
}

// We need to insert R_RISCS_ALIGN relocation type to indicate the
// position of Nops and the total bytes of the Nops have been inserted
// when linker relaxation enabled.
// The function insert fixup_RISCS_align fixup which eventually will
// transfer to R_RISCS_ALIGN relocation type.
bool RISCSAsmBackend::shouldInsertFixupForCodeAlign(MCAssembler &Asm,
                                                    MCAlignFragment &AF) {
...
RISCSAsmBackend::createObjectTargetWriter() const {
  return createRISCSELFObjectWriter(OSABI, Is64Bit);
}

MCAsmBackend *llvm::createRISCSAsmBackend(const Target &T,
                                          const MCSubtargetInfo &STI,
                                          const MCRegisterInfo &MRI,
                                          const MCTargetOptions &Options) {
  const Triple &TT = STI.getTargetTriple();
  uint8_t OSABI = MCELFObjectTargetWriter::getOSABI(TT.getOS());
  return new RISCSAsmBackend(STI, OSABI, TT.isArch64Bit(), Options);
}
