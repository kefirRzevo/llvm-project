...
#define DEBUG_TYPE "riscs-lowering"
...
RISCSTargetLowering::RISCSTargetLowering(const TargetMachine &TM,
  ...
  setStackPointerRegisterToSaveRestore(riscs::X2);
...
const char *RISCSTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case riscsISD::CALL:
    return "riscsISD::CALL";
  case riscsISD::RET_FLAG:
    return "riscsISD::RET";
  }
  return nullptr;
}
...
#include "RISCSGenCallingConv.inc"
...
void RISCSTargetLowering::analyzeOutputArgs(
    MachineFunction &MF, CCState &CCInfo,
    const SmallVectorImpl<ISD::OutputArg> &Outs, bool IsRet,
    CallLoweringInfo *CLI, RISCSCCAssignFn Fn) const {
...
void RISCSTargetLowering::analyzeInputArgs(
    MachineFunction &MF, CCState &CCInfo,
...
SDValue RISCSTargetLowering::LowerCall(CallLoweringInfo &CLI,
  ...
  analyzeOutputArgs(MF, ArgCCInfo, Outs, /*IsRet=*/false, &CLI, CC_RISCS);
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, riscs::X2, PtrVT);
      SDValue Address =
  ...
  if (GlobalAddressSDNode *S = dyn_cast<GlobalAddressSDNode>(Callee)) {
    ...
    unsigned OpFlags = riscsII::MO_CALL;
    if (!getTargetMachine().shouldAssumeDSOLocal(GV))
      OpFlags = riscsII::MO_PLT;
  ...
  Chain = DAG.getNode(riscsISD::CALL, DL, NodeTys, Ops);
  ...
  analyzeInputArgs(MF, RetCCInfo, Ins, /*IsRet=*/true, CC_RISCS);
...
/// riscs formal arguments implementation
SDValue RISCSTargetLowering::LowerFormalArguments(
  ...
  analyzeInputArgs(MF, CCInfo, Ins, /*IsRet=*/false, CC_RISCS);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &riscs::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    RISCSFunctionInfo *RVFI = MF.getInfo<RISCSFunctionInfo>();
...
RISCSTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  unsigned RetOpc = riscsISD::RET_FLAG;
...
bool RISCSTargetLowering::mayBeEmittedAsTailCall(const CallInst *CI) const {
  return false;
}
...
SDValue RISCSTargetLowering::lowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(riscsISD::BR_CC, DL, Op.getValueType(), Op.getOperand(0),
