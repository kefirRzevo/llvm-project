...
#define DEBUG_TYPE "riscs-frame-lowering"
...
void RISCSFrameLowering::determineCalleeSaves(MachineFunction &MF,
  ...
  // pointer.
  if (hasFP(MF)) {
    SavedRegs.set(riscs::X1);
    SavedRegs.set(riscs::X8);
  }
  // Mark BP as used if function has dedicated base pointer.
  // RISCS doesn't support varargs yet, so no need base pointer
  // This peace of code if for future development
  if (hasBP(MF))
    SavedRegs.set(riscsABI::getBPReg());
}
...
void RISCSFrameLowering::adjustReg(MachineBasicBlock &MBB,
  ...
  if (isInt<12>(Val)) {
    BuildMI(MBB, MBBI, DL, TII->get(riscs::ADDI), DestReg)
        .addReg(SrcReg)
        .addImm(Val)
        .setMIFlag(Flag);
  } else {
...
void RISCSFrameLowering::adjustStackToMatchRecords(
...
static Register getFPReg(const RISCSSubtarget &STI) { return riscs::X8; }

// Returns the register used to hold the stack pointer.
static Register getSPReg(const RISCSSubtarget &STI) { return riscs::X2; }

// Determines the size of the frame and maximum call frame size.
void RISCSFrameLowering::determineFrameLayout(MachineFunction &MF) const {
...
void RISCSFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  Register BPReg = riscsABI::getBPReg();
  ...
  if (!isInt<16>(StackSize)) {
    llvm_unreachable("Stack offs won't fit in riscs::LDi");
  }
  ...
  if (RI->hasStackRealignment(MF)) {
    ...
    if (hasBP(MF)) {
      // move BP, SP
      BuildMI(MBB, MBBI, DL, TII->get(riscs::ADDI), BPReg)
          .addReg(SPReg)
          .addImm(0)
          .setMIFlag(MachineInstr::FrameSetup);
    }
...
bool RISCSFrameLowering::spillCalleeSavedRegisters(
...
bool RISCSFrameLowering::restoreCalleeSavedRegisters(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MI,
...
void RISCSFrameLowering::processFunctionBeforeFrameFinalized(
...
MachineBasicBlock::iterator RISCSFrameLowering::eliminateCallFramePseudoInstr(
  ...
  if (!hasReservedCallFrame(MF)) {
    ...
    if (Amount != 0) {
      ...
      if (MI->getOpcode() == riscs::ADJCALLSTACKDOWN)
        Amount = -Amount;
...
bool RISCSFrameLowering::hasFPImpl(const MachineFunction &MF) const {
...
bool RISCSFrameLowering::hasBP(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
...
RISCSFrameLowering::getFrameIndexReference(const MachineFunction &MF, int FI,
  ...
  if (FI >= MinCSFI && FI <= MaxCSFI) {
    ...
    // Realigned stack
    if (hasBP(MF)) {
      // This branch is not chosen, as RISCS doesn't support varargs yet.
      FrameReg = riscsABI::getBPReg();
    } else {
...
bool RISCSFrameLowering::hasReservedCallFrame(const MachineFunction &MF) const {
