//===-- RISCSISAInfo.cpp - RISC-V Arch String Parser ------------*- C++ -*-===//
//
...
struct RISCSSupportedExtension {
  const char *Name;
  /// Supported version.
  RISCSISAInfo::ExtensionVersion Version;

  bool operator<(const RISCSSupportedExtension &RHS) const {
...
static const RISCSSupportedExtension SupportedExtensions[] = {
    {"a", {2, 1}},
    {"d", {2, 2}},
    {"i", {2, 1}},
    {"m", {2, 0}},
...
namespace {
struct LessExtName {
  bool operator()(const RISCSSupportedExtension &LHS, StringRef RHS) {
    return StringRef(LHS.Name) < RHS;
  }
  bool operator()(StringRef LHS, const RISCSSupportedExtension &RHS) {
    return LHS < StringRef(RHS.Name);
  }
};
} // namespace

static std::optional<RISCSISAInfo::ExtensionVersion>
...
void RISCSISAInfo::addExtension(StringRef ExtName,
                                 RISCSISAInfo::ExtensionVersion Version) {
...
bool RISCSISAInfo::isSupportedExtensionFeature(StringRef Ext) {
  ArrayRef<RISCSSupportedExtension> ExtInfo =
...
bool RISCSISAInfo::isSupportedExtension(StringRef Ext) {
  verifyTables();
...
bool RISCSISAInfo::isSupportedExtension(StringRef Ext, unsigned MajorVersion,
...
bool RISCSISAInfo::hasExtension(StringRef Ext) const {
...
bool RISCSISAInfo::compareExtension(const std::string &LHS,
...
std::vector<std::string> RISCSISAInfo::toFeatures(bool AddAllExtensions,
  ...
  if (AddAllExtensions) {
    for (const RISCSSupportedExtension &Ext : SupportedExtensions) {
...
static Error getExtensionVersion(StringRef Ext, StringRef In, unsigned &Major,
  ...
  if (RISCSISAInfo::isSupportedExtension(Ext, Major, Minor))
    return Error::success();
...
llvm::Expected<std::unique_ptr<RISCSISAInfo>>
RISCSISAInfo::parseFeatures(unsigned XLen,
                             const std::vector<std::string> &Features) {
  assert(XLen == 64);
  std::unique_ptr<RISCSISAInfo> ISAInfo(new RISCSISAInfo(XLen));
  ...
  return RISCSISAInfo::postProcessAndChecking(std::move(ISAInfo));
}

llvm::Expected<std::unique_ptr<RISCSISAInfo>>
RISCSISAInfo::parseNormalizedArchString(StringRef Arch) {
...
static Error processSingleLetterExtension(
    StringRef &RawExt,
    MapVector<std::string, RISCSISAInfo::ExtensionVersion,
  ...
  if (IgnoreUnknown && !RISCSISAInfo::isSupportedExtension(Name))
    return Error::success();
...
llvm::Expected<std::unique_ptr<RISCSISAInfo>>
RISCSISAInfo::parseArchString(StringRef Arch, bool IgnoreUnknown) {
  ...
  for (auto &SeenExtAndVers : SeenExtMap) {
    ...
    RISCSISAInfo::ExtensionVersion ExtVers = SeenExtAndVers.second;

    if (!RISCSISAInfo::isSupportedExtension(ExtName)) {
      if (ExtName.size() == 1) {
        return createStringError(errc::invalid_argument,
...
Error RISCSISAInfo::checkDependency() {
  return Error::success();
}
...
void RISCSISAInfo::updateImplication() {
...
void RISCSISAInfo::updateMinVLen() {
  for (auto const &Ext : Exts) {
...
std::string RISCSISAInfo::toString() const {
  std::string Buffer;
...
llvm::Expected<std::unique_ptr<RISCSISAInfo>>
RISCSISAInfo::postProcessAndChecking(std::unique_ptr<RISCSISAInfo> &&ISAInfo) {
...
StringRef RISCSISAInfo::computeDefaultABI() const {
  if (XLen == 64) {
    if (hasExtension("d"))
      return "lp64d";
